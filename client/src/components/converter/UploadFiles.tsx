    ...
    <div className="p-8">
      ...
      {!importedRepo ? (
        ...
        <div className="space-y-6">
          ...
          {/* Analysis Section */}
          {isAnalyzing ? (
            <div className="border-2 border-gray-200 rounded-xl p-12 flex flex-col items-center justify-center">
              <Loader2 size={48} className="text-indigo-500 animate-spin mb-4" />
              <h3 className="text-xl font-semibold mb-2">Analyzing Repository</h3>
              <p className="text-gray-500">Scanning files and extracting structure...</p>
            </div>
          ) : analysisData ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="border-2 border-gray-200 rounded-xl p-6 bg-white">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between py-3 border-b border-gray-100">
                      ...
                      <span className="font-bold text-gray-900">{analysisData.total_files}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between py-3 border-b border-gray-100">
                      ...
                      <span className="font-bold text-gray-900">{analysisData.functions_count}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between py-3 border-b border-gray-100">
                      ...
                      <span className="font-bold text-gray-900">{analysisData.classes_count}</span>
                    </div>
                    
                    {analysisData.summary.stars !== undefined && (
                      <div className="flex items-center justify-between py-3 border-b border-gray-100">
                        <span className="text-gray-600 flex items-center gap-2">
                          <Star size={16} />
                          Stars
                        </span>
                        <span className="font-bold text-gray-900">{analysisData.summary.stars}</span>
                      </div>
                    )}
                    
                    {analysisData.summary.forks !== undefined && (
                      <div className="flex items-center justify-between py-3">
                        <span className="text-gray-600 flex items-center gap-2">
                          <GitFork size={16} />
                          Forks
                        </span>
                        <span className="font-bold text-gray-900">{analysisData.summary.forks}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Languages Card */}
                {analysisData.summary.languages.length > 0 && (
                  <div className="border-2 border-gray-200 rounded-xl p-6 bg-white">
                    <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {analysisData.summary.languages.map((lang, idx) => (
                        <span
                          key={idx}
                ...
                {analysisData.summary.license && (
                  <div className="border-2 border-gray-200 rounded-xl p-6 bg-white">
                    ...
                    <div className="space-y-2">
                      <p className="font-medium text-gray-900">{analysisData.summary.license.name}</p>
                      <a
                        href={analysisData.summary.license.url}
                        target="_blank"
                  ...
                  <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-semibold flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-500 font-normal ml-2">
                          ({analysisData.files.length} files)
                        </span>
                      </h3>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-4 py-2 pl-10 bg-white border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                      />
                  ...
                  <div className="p-4 max-h-[600px] overflow-y-auto">
                    {analysisData.files.length > 0 ? (
                      <div className="space-y-0.5">
                        {filterTree(fileTree, searchQuery).map((node, idx) => (
                          <FileTreeNode key={`${node.path}-${idx}`} node={node} level={0} />
                        ))}
                        {searchQuery && filterTree(fileTree, searchQuery).length === 0 && (
            ...
            <div className="border-2 border-gray-200 rounded-xl p-12 flex flex-col items-center justify-center">
              ...
              <p className="text-gray-500 mb-6">Click below to start analyzing the repository structure</p>
              <button
                onClick={() => handleAnalyzeRepository()}
                className="bg-indigo-500 text-white px-8 py-3 rounded-lg hover:bg-indigo-600 transition-colors flex items-center gap-2 font-medium"
              >
                Analyze Repository
                <ChevronRight size={20} />
              </button>
